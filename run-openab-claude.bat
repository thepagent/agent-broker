...
set LOGDIR=C:\Users\Administrator\openab\logs
...
2>nul (
  9>"%LOCKFILE%" (
    :loop
    "C:\Users\Administrator\openab\target\release\openab.exe" config.toml >> "%LOGDIR%\openab.log" 2>&1
    %SYSTEMROOT%\System32\timeout.exe /t 5 /nobreak > nul
    goto loop
  )
) || (
  echo Another loop is already running for this bot. Exiting.
  exit /b 0
)
